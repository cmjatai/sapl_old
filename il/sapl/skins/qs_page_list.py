...
linkString = '<A data="%size" title="%total" class="%css" href="' + linkLocation + "?p=%s&amp;s=%step"
...
if showFirst:
    if page >= 2:
       linkStringTemp = 'p=1'.join(linkString.split('p=%s'))
...
if showPrevious:
    if page > 1:
        linkStringTemp = ('p=' + str(second)).join(linkString.split('p=%s'))
...
if showNext:
    if page < numPages:
        linkStringTemp = ('p=' + str(fourth)).join(linkString.split('p=%s'))
...
if showLast:
    if page <= numPages - 1:
        linkStringTemp = ('p=' + str(numPages)).join(linkString.split('p=%s'))
...
if 'printer' in request and len(request['printer']) == 0:
    ...
    linkStringTemp = 'p=1'.join(printString.split('p=%s'))
