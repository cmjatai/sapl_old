...
id_odt = str(cod_materia) + '_texto_integral.odt'
id_prop_odt = str(cod_proposicao) + '.odt'
try:
    ...
    if (tip_prop == 'XML'):
        ...
        if hasattr(context.sapl_documentos.proposicao, id_prop_odt):
            tmp_copy_odt = context.sapl_documentos.proposicao.manage_copyObjects(ids=str(cod_proposicao)+'.odt')
            tmp_id_odt = context.sapl_documentos.materia_odt.manage_pasteObjects(tmp_copy_odt)[0]['new_id']
            context.sapl_documentos.materia.manage_renameObjects(ids=list([tmp_id_odt]),new_ids=list([id_odt]))
        if hasattr(context.sapl_documentos.proposicao, str(cod_proposicao)):
            tmp_copy = context.sapl_documentos.proposicao.manage_copyObjects(ids=str(cod_proposicao))
            tmp_id = context.sapl_documentos.materia.manage_pasteObjects(tmp_copy)[0]['new_id']
            context.sapl_documentos.materia.manage_renameObjects(ids=list([tmp_id]),new_ids=list([id]))
    ok = 1
