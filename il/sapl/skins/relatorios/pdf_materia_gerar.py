...
def cabecalho(dic_cabecalho,imagem):
    ...
    if len(dic_cabecalho['total_materias'])>0:
        tmp_data+='\t\t\t\t<lines>2cm 3.2cm 19cm 3.2cm</lines>\n'
        tmp_data+='\t\t<para style="P2"><b>Total de Matérias: </b>' + dic_cabecalho['total_materias'] +' </para>\n'
...
def materias(lst_materias):
    ...
    for dic in lst_materias:
        ...
        tmp_data+='\t\t<para style="P3"> <b>Autor: </b>'
        autoria=[]
        autoria=dic['autoria_materia']
        j=0
        for i in range(len(autoria)):
            if j==0:
                j=1
            else:
            ...
            y=autoria[i].keys()[0]
            primeiro_autor=autoria[i].get(y, "falha-primeiro-autor")[0] # = 1- primeiro autor, 2-não
            nom_autor=autoria[i].get(y, "falha-nom-autor")[1]
            tipo_autor=autoria[i].get(y, "falha-tipo-autor")[3]
            tmp_data+=nom_autor + ' - '
            if tipo_autor!='Mesa Diretora' and tipo_autor!='Prefeito':
                tmp_data+=tipo_autor
            if primeiro_autor=="1":
                tmp_data+=" - 1o. autor "
            elif tipo_autor!='não definido':
                tmp_data+=" - co-autor "
            tmp_data+=' </para>\n'

        if dic['txt_ementa']!=None:
            tmp_data+='\t\t<para style="P3"><b>Ementa: </b>'
            txt_ementa = dic['txt_ementa'].replace('&','&amp;')
            tmp_data+=txt_ementa +' </para>\n'

        if dic['des_situacao']!=None:
            tmp_data+='\t\t<para style="P3"><b>Situação: </b>' + dic['des_situacao'] +' </para>\n'

        if dic['norma_vinculada']!=None:
            tmp_data+='\t\t<para style="P3"><b>Norma Jurídica Vinculada:</b>' + dic['norma_vinculada'] +' </para>\n'

        if dic['localizacao_atual']!=None:
            tmp_data += '\t\t<para style="P3"><b>Local atual: </b>'
            txt_local = dic['localizacao_atual'].replace('&','&amp;')
            tmp_data += txt_local + '</para>\n'

        if dic['ultima_acao']!=None:
            tmp_data+='\t\t<para style="P3"><b>Última ação: </b>' + dic['ultima_acao'] +' </para>\n'
