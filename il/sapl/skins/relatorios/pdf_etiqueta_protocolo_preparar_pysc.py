...
use_spdo = context.sapl_documentos.props_sapl.use_spdo
protocolos=[]
REQUEST=context.REQUEST
for protocolo in context.zsql.protocolo_pesquisar_zsql(tip_protocolo=REQUEST['rad_tip_protocolo'],
    ...
    for materia in context.zsql.materia_obter_zsql(num_protocolo=protocolo.num_protocolo,ano_ident_basica=protocolo.ano_protocolo):
        ...
        if materia.sgl_tipo_materia == 'REQ':
            dic['num_materia'] = materia.des_tipo_materia.upper() + ' '+str(materia.num_ident_basica)+'/'+str(materia.ano_ident_basica)
        else:
            dic['num_materia'] = materia.sgl_tipo_materia.upper() + ' '+str(materia.num_ident_basica)+'/'+str(materia.ano_ident_basica)
